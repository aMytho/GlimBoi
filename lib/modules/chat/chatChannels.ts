...
function addRecentChannel(channel: string, timestamp = null, autoJoin = false): Promise<channel> {
    ...
    return new Promise(done => {
        recentChannelsDB.find({ channel: channel }, function (err: string, doc: string | any[]) {
            if (doc.length == 0) {
                console.log(`Adding channel ${channel} to recent channels`);
                recentChannelsDB.insert({ channel: channel, timestamp: timestamp, autoJoin: autoJoin }, function (err, doc: channel) {
                    console.log(doc);
                    done(doc);
                });
            } else {
