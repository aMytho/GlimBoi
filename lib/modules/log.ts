...
class LoggingEvent implements LogType {
     ...
     * @returns {string}
     */
    determineDescription(event: string, caused:userName, affected:string[], data:any) {
        switch (event) {
            case "Add User":
...
function getLogByID(id:string) {
    loggingDB.find({event: id}, function (err: Error | null, doc) {
        console.log("Event logged", + doc)
  });
}
