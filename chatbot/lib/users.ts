...
import * as LogHandle from "LogHandle"
 ...
 */
async function addUser(user:string, inModal: boolean, createdBy: string = "Glimboi") {
        ...
        usersDB.find({ userName: user }, async function (err:string, docs:docs) {
            if (docs.length == 0) {
                ...
                if (typeof ID !== "number" || ID == null || typeof ID == "object") {
                    ...
                    usersDB.insert(tempUser, function (err:Error | null, doc:userDoc) {
                        console.log(doc);
 ...
 */
async function removeUser(user:string, inModal:boolean, userWhoRemoves = "Glimboi") {
 ...
 * @param {number} id
 */
async function addQuote(quote:QuoteType, id:number) {
  	return new Promise(resolve => {
 ...
 * @async
 */
async function removeQuoteByID(id:number, user:string) {
  	return new Promise(resolve => {
    		...
    		if (docs.length < 1) {
      			...
      			for (let index = 0; index < docs[0].quotes.length; index++) {
        			if (docs[0].quotes[index].quoteID == id) {
          				...
          				usersDB.update({$and: [{userName: user}, {quotes: {$elemMatch: {quoteID: id}}}]}, {$set: {quotes: docs[0].quotes}}, {returnUpdatedDocs: true}, function(affectedDocuments:UserDocs) {
 ...
 * @param {Array} Users
 */
function earnPointsWT(Users:User[]) {
 ...
 */
function addPoints(user:userName, points:number) {
 ...
 */
function removePoints(user:userName, value:number) {
